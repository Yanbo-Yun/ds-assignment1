...
export class DsAssignment1Stack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const animals = api.root.addResource('animals');

    animals.addMethod('POST', new apig.LambdaIntegration(postFn));

    animals.addMethod('GET', new apig.LambdaIntegration(getItemsFn))

    const ItemById = animals.addResource('{Id}');
    ItemById.addMethod('GET', new apig.LambdaIntegration(getItemFn), );

    animals.addMethod('PUT',new apig.LambdaIntegration(updateFn));
